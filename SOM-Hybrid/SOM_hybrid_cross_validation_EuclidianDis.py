...
# -------------------- CLASSE SOM HÍBRIDO (VARIAÇÃO EUCLIDIANA NO TESTE) --------------------
class SOMHybrid:
    ...
    # Distância Ponderada (Usada no Treino)
    ...
    # NOVO: Distância Euclidiana Padrão (Usada APENAS no Teste/Predict)
    def _calculate_euclidean_dists_sq(self, X, W):
        """Calcula ||X - W||^2 sem ponderação lambda."""
    ...
    def fit(self, X, y):
        ...
        while not initialization_success:
            ...
            # Inicialização usa Lambda (que é 1 aqui, então igual euclidiana)
        ...
        while t < self.max_iter:
            ...
            # 2) Competição (USA LAMBDA PARA TREINO)
            ...
            # Custo Final (usando métrica original de treino)
            reg_sq = self._calculate_regression_errors_sq(X_prime, y_vec, self.betas)
            quant_sq = self._calculate_weighted_quantization_dist_sq(X, self.prototypes, self.lambdas)
            gen_err = self._calculate_total_generalized_error(reg_sq, quant_sq, H, self.alpha)
    ...
    # --- PREDICT (MODIFICADO: PROTÓTIPO MAIS PRÓXIMO VIA EUCLIDIANA) ---
    def predict(self, X_test):
        """
        1. Encontra o protótipo W mais próximo (Distância Euclidiana Padrão - Ignora Lambda).
        2. Usa os coeficientes Beta do vencedor para calcular Mu.
        """
        N_test = X_test.shape[0]
        
        # --- ALTERAÇÃO AQUI: USA DISTÂNCIA EUCLIDIANA ---
...
if __name__ == "__main__":
    print("--- SOM Híbrido (CV: Teste via Euclidiana, Treino via Lambda) ---")
    ...
    for rep in range(n_cv_reps):
        ...
        for train_index, test_index in kf.split(X_final):
            ...
            # 2. Treina Modelo (Usa Lambda para aprender topologia)
            som_seed = (rep * 100) + fold_count 
            som = SOMHybrid(mh, mw, iters, h0, hf, random_state=som_seed)
            som.fit(X_train, y_train)
            
            # 3. Prediz (Usa Euclidiana para encontrar vencedor)
    ...
    print("\n" + "="*50)
    print("   RESULTADO FINAL (SOM Híbrido - Teste Euclidiano)")
    print("="*50)
    ...
    with open("somhybrid_euclid_test_stats.txt", "w", encoding="utf-8") as f_out:
        f_out.write(f"--- RELATORIO SOM-HYBRID (TESTE EUCLIDIANO) ---\n")
    ...
    print("\nRelatório estatístico salvo em 'somhybrid_euclid_test_stats.txt'.")