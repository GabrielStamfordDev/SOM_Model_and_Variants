...
# Função Global para Distância Euclidiana (Reutilizada para comparação)
def calculate_global_euclidean_matrix(X):
    """
    Calcula a matriz de distâncias quadradas euclidianas entre TODOS os pontos.
    (a-b)^2 = a^2 + b^2 - 2ab
    """
...
# -------------------- CLASSE SOM HÍBRIDO (MANTIDA ORIGINAL) --------------------
# O treino continua usando Lambda para definir a topologia correta
class SOMHybrid:
...
if __name__ == "__main__":
    print("--- SOM Híbrido (CV k-NN com Distância Euclidiana Global) ---")
    print("Nota: Treinamento usa Lambda; Predição k-NN usa Euclidiana.")
    ...
    # =========================================================
    #    CÁLCULO DA MATRIZ DE DISTÂNCIAS GLOBAL (Euclidiana)
    # =========================================================
    print("\nCalculando Matriz de Distâncias Global (Euclidiana Fixa)...")
    # Calculamos N x N uma vez para usar na predição
    ...
    for rep in range(n_cv_reps):
        ...
        for train_index, test_index in kf.split(X_final):
            ...
            # 2. Treina SOM Híbrido (USA LAMBDA NO TREINO)
            # O modelo aprende os clusters (labels) usando a distância ponderada
            som = SOMHybrid(mh, mw, iters, h0, hf, random_state=(rep * 100) + fold_count)
            som.fit(X_train, y_train)
            
            train_labels = som.labels_
            
            # 3. Predição k-NN usando Matriz Global EUCLIDIANA (IGNORA LAMBDA)
            # Recortamos a submatriz Euclidiana
            ...
            for i in range(len(test_index)):
                ...
                # Ordena índices dos vizinhos (distância euclidiana)
                ...
                for k_neighbors in k_grid:
                    ...
                    # Predição com Beta do Vencedor (Aprendido no treino híbrido)
                ...
                # Seleciona o melhor k para este dado
        ...
        print(f"   [Concluído] RMSE (Euclidean k-NN) da Repetição {rep+1}: {rep_rmse:.4f}")
    ...
    print("\n" + "="*50)
    print("   RESULTADO FINAL (SOM Híbrido + Euclidiana k-NN)")
    print("="*50)
    print(f"RMSE Médio: {mean_rmse:.6f}")
    print(f"Desvio Padrão RMSE: {std_rmse:.6f}")
    
    with open("somhybrid_euclidean_knn_cv_stats.txt", "w", encoding="utf-8") as f_out:
        f_out.write(f"--- RELATORIO SOM-HYBRID (EUCLIDEAN KNN) ---\n")
    ...
    print("\nRelatório salvo em 'somhybrid_euclidean_knn_cv_stats.txt'.")